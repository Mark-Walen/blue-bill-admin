    ...
    <div class="dynamic-dialog-wrapper">
        <el-dialog
            ...
            <template #header="{ close, titleId, titleClass }"
                      style="--el-dialog-padding-primary: 0; padding-bottom: 0; margin-right: 0;">
                <div class="wrapper">
                    <bm-bar>
                        <span class="font-bold text-caption" :id="titleId">{{ props.options && props.options.title
                            }}</span>
                        <div class="space"></div>
                        <el-button @click="close" link class="btn__dialog-close">
                            <span class="focus-helper"></span>
                            <svg-icon :size="18" name="close"></svg-icon>
                        </el-button>
                    </bm-bar>
                </div>
            </template>
            ...
            <template #footer>
                <component :is="props.options.footerComp" v-bind="props.options.footerComProps" @done="doneHandler"></component>
            </template>
        </el-dialog>
    </div>
</template>
